    ...
    inquirer.prompt([
        {
            type: 'input',
            name: 'MongoDB',
            message: 'Please provide a mongodb uri',
        ...
        },
        {
            type: 'input',
            name: 'port',
            message: 'If you would like to run DT_SMPNL on a custom port please enter that port below!',
            default: 8080
        }
    ]).then(answers => {
        const envobj = {
...
function runBuild() {
    ...
    let chield = ex('npm run build').then(async (result) => {
        ...
        console.log(`DT_SMPNL has been successfully built`)
    });
